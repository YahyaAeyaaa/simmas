// app/admin/pengguna/components/EditUser.tsx
"use client";

import { useState, useEffect } from "react";
import { X, User, Mail, UserCircle, Shield } from "lucide-react";
...
export default function ModalEditUser({ isOpen, onClose, onSubmit, userData }: ModalEditUserProps) {
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          <div className="space-y-4">
            ...
            <FormInput label="Email" name="email" type="email" value={formData.email} onChange={(v) => handleChange("email", v)} placeholder="user@example.com" icon={User} error={errors.email} disabled={isLoading} required />
            ...
            {formData.role === 'siswa' && (
              <>
                <FormInput label="NIS" name="nis" type="text" value={formData.nis} onChange={(v) => handleChange("nis", v)} placeholder="Masukkan NIS" icon={User} error={errors.nis} disabled={isLoading} required />
                <FormInput label="Jurusan" name="jurusan" type="text" value={formData.jurusan} onChange={(v) => handleChange("jurusan", v)} placeholder="Masukkan jurusan" icon={User} error={errors.jurusan} disabled={isLoading} required />
            ...
            <FormAlert type="warning" title="Catatan :" message="Untuk mengubah password silahkan gunakan fitur reset password yang tersedia" />
          </div>
          ...
          <div className="flex gap-3 mt-6 pt-4 border-t border-gray-200">
            ...
            <button type="submit" disabled={isLoading} className="flex-1 px-4 py-2 bg-gradient-to-r from-cyan-400 to-sky-700 text-white rounded-lg hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2">
              {isLoading ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Menyimpan...</>) : ("Simpan Perubahan")}
            </button>
          </div>
        </form>
      </div>
